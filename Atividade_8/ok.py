...
# Estados e Lista de Palavras
LOGIN, MENU, CASA, FORCA = 0, 1, 2, 3
LISTA_PALAVRAS = ["PUCPR", "PYTHON", "PROGRAMACAO", "TECNOLOGIA", "ALGORITMO", "LOGICA", "CURITIBA"]
...
def desenhar_casa_tela(nuvem_x, mouse_pos):
    TELA.fill(AZUL_CEU)
    pygame.draw.rect(TELA, VERDE, (0, 500, 800, 100))
    mx, my = mouse_pos
    pygame.draw.circle(TELA, AMARELO, (mx, my), 40)
    for offset in [0, 400]:
        x = (nuvem_x + offset) % LARGURA
        pygame.draw.circle(TELA, BRANCO, (int(x), 100), 30)
        pygame.draw.circle(TELA, BRANCO, (int(x) + 40, 100), 40)
        pygame.draw.circle(TELA, BRANCO, (int(x) + 80, 100), 30)
    pygame.draw.rect(TELA, (255, 230, 150), (250, 250, 300, 250))
    pygame.draw.polygon(TELA, VERMELHO, [(230, 250), (400, 100), (570, 250)])
    pygame.draw.rect(TELA, MARROM, (370, 380, 60, 120))
    pygame.draw.rect(TELA, BRANCO, (280, 300, 60, 60))

def desenhar_forca(erros):
    pygame.draw.line(TELA, PRETO, (100, 500), (300, 500), 5)
    pygame.draw.line(TELA, PRETO, (200, 500), (200, 150), 5)
    pygame.draw.line(TELA, PRETO, (200, 150), (350, 150), 5)
    pygame.draw.line(TELA, PRETO, (350, 150), (350, 200), 5)
    partes = [
        lambda: pygame.draw.circle(TELA, PRETO, (350, 230), 30, 5),
        lambda: pygame.draw.line(TELA, PRETO, (350, 260), (350, 400), 5),
        lambda: pygame.draw.line(TELA, PRETO, (350, 300), (300, 350), 5),
        lambda: pygame.draw.line(TELA, PRETO, (350, 300), (400, 350), 5),
        lambda: pygame.draw.line(TELA, PRETO, (350, 400), (310, 480), 5),
        lambda: pygame.draw.line(TELA, PRETO, (350, 400), (390, 480), 5),
    ]
...
class InputBox:
    def __init__(self, x, y, w, h, label, segredo=False):
        self.rect = pygame.Rect(x, y, w, h); self.text, self.label, self.active, self.segredo = "", label, False, segredo
    ...
    def draw(self):
        lbl = FONTE.render(self.label, True, PRETO); TELA.blit(lbl, (self.rect.x, self.rect.y - 30))
...
def main():
    ...
    nuvem_x = 0; msg_erro = ""
    letras = []; erros = 0
    palavra = random.choice(LISTA_PALAVRAS) # Sorteio inicial

    while True:
        TELA.fill(BRANCO); m_pos = pygame.mouse.get_pos()
        for event in pygame.event.get():
            ...
            if estado == LOGIN:
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE: # Reset ao sair
                        estado = MENU; erros = 0; letras = []; palavra = random.choice(LISTA_PALAVRAS)
                    elif event.unicode.isalpha():
                        L = event.unicode.upper()
                        if L not in letras:
                            letras.append(L)
                            if L not in palavra: erros += 1
        ...
        if estado == LOGIN:
            ...
            if pygame.key.get_pressed()[pygame.K_ESCAPE]: estado = MENU
        elif estado == FORCA:
            ...
            if erros >= 6 or all(l in letras for l in palavra):
                txt = "GANHOU! Pressione ESC" if erros < 6 else f"PERDEU! Palavra: {palavra}"
