...
dados_h2 = [100,120,130,120,150,100,160,200,190,110,115,125,135,170,130]
...
while rodando:
    ...
    # Tela de login inicial (apenas para estética, sem funcionalidade real)
    if histograma_atual == 1 and len(dados_h1) == 0:
        txt_login = fonte_g.render("Bem-vindo! Gerando dados para o Histograma 1...", True, COR_TEXTO)
        tela.blit(txt_login, (LARGURA // 2 - txt_login.get_width() // 2, ALTURA // 2 - txt_login.get_height() // 2))
        display.flip()
        time.delay(2000)  # Simula um carregamento inicial
        continue

    # Menu Interativo para Navegação entre os Histogramas
    if histograma_atual == 1:
        txt_menu = fonte_p.render("Histograma 1: Lista Aleatória (Automática)", True, COR_TEXTO)   
    elif histograma_atual == 2:
        txt_menu = fonte_p.render("Histograma 2: Lista Estática com Quantidades Aleatórias", True, COR_TEXTO)
    elif histograma_atual == 3:
        txt_menu = fonte_p.render("Histograma 3: Input do Usuário (Via PyGame)", True, COR_TEXTO)
    tela.blit(txt_menu, (LARGURA // 2 - txt_menu.get_width() // 2, 10))
